		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {			
				try {
					...
					PreparedStatement pst = conn.prepareStatement("SELECT nextval('CUSTOMERS_SEQUENCE')");
					synchronized( this ) {
						...
						rs = st.executeQuery("select * from ADDRESS where COUNTRY='"+ country.getText()+"'" + " AND STATE = '" + state.getText() +  "' AND CITY = '" + city.getText() + "'"
								+ " AND ZIP = '" + zip.getText() + "' AND STREET = '" + street.getText() + "'");
						...
						st.executeUpdate("INSERT INTO CUSTOMER VALUES ('" + myId + "', '" + firstname.getText() + "', '" + lastname.getText() + "', '"
								+ password.getText()+"', '"+ addressId +"', '"+ branchId + "')");
